...
$res = mysqli_query($db, "SELECT id FROM visits WHERE date='$date'") or die ("Проблема при подключении к БД");
...
else
{
    ...
    $current_ip = mysqli_query($db, "SELECT id FROM ips WHERE ip_address='$visitor_ip'");
